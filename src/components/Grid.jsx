...
export default function Grid() {
    ...
    function handleSave (index,data) {
       ...
       localStorage.setItem('fitprogram', JSON.stringify(newObj))
    ...
    useEffect(() => {
        ...
        let savedData = {}
        if (localStorage.getItem('fitprogram')) {
            savedData = JSON.parse(localStorage.getItem('fitprogram'))
        }
    ...
    return (
       <div className="training-plan-grid">
            { Object.keys(training_plan).map((workout,workoutIndex) => {
                ...
                const dayNum = (workoutIndex + 1 < 10) ? '0'+ (workoutIndex + 1 ) : workoutIndex + 1
               ...
               if (workoutIndex === selectedWorkout) {
                ...
                return(
                    <button onClick={() => {
                        if (isLocked) {return}
                        setSelectedWorkout(workoutIndex)}} className={'card plan-card ' + (isLocked ? 'inactive' : '' ) } key={workoutIndex}>
...
//Day {((workoutIndex / 8) <= 1) ? '0'+ (workoutIndex + 1 ) : workoutIndex + 1 } 

/* 
workoutIndex % 3 === 0 first-day: pull
workoutIndex % 3 === 1 second-day:push
workoutIndex % 3 !==1 and !==0   third-day:legs
*/ 